...
from model.losses import get_masked_ptc_loss, get_seg_loss, CTCLoss_neg, DenseEnergyLoss, get_energy_loss,CPCLoss,get_spacial_bce_loss,ContrastLoss_moco,ContrastLoss_prototype,get_bg_fg_contrastive_loss,get_bg_fg_contrastive_clamp_loss
...
from utils.camutils import single_bg_fg_crop_denoise,single_bg_fg_crop_denoise_v2,single_bg_fg_crop,single_class_crop,cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads
...
parser.add_argument("--local_crop_num", default=2, type=int, help="crop_num")
...
def train(args=None):
    ...
    model = network_du_heads_independent_config_cl_gl(
...
#pretrained_load——————————————————————————————————————————————————————————————————————————————————————————————————
    # trained_state_dict = torch.load('/home/zhonggai/python-work-space/DEFormer/DEFormer/scripts/work_dir_voc_wseg/75.2(num = 2 crop = 96)/checkpoints/default_model_iter_8000.pth', map_location="cpu")
...
#get_local_pic_to_encode------------------------------------------------------------------------------
        ...
        b1_local_crops, b1_flags, b1_box= single_bg_fg_crop_denoise_v2(images=inputs, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, roi_mask = b1_roi_mask)
        b2_local_crops, b2_flags, b2_box= single_bg_fg_crop_denoise_v2(images=inputs, cls_label = cls_label,roi_mask_28x=b2_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, roi_mask = b2_roi_mask)
        local_pic = (b1_local_crops , b2_local_crops)
...
#contrast learning -----------------------------------------------------------------------
        ...
        if n_iter >= 3000:
            b1_bg_contrast_loss = get_bg_fg_contrastive_clamp_loss(b1_cls_token.detach(),b1_contrast_feature,num_crop=n_crops) 
            b2_bg_contrast_loss = get_bg_fg_contrastive_clamp_loss(b2_cls_token.detach(),b2_contrast_feature,num_crop=n_crops) 
            contrast_loss = b1_bg_contrast_loss + b2_bg_contrast_loss
            # print("Contrast",contrast_loss)   
        else:
            contrast_loss = torch.tensor(0)
