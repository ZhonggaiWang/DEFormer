...
#pretrained_load——————————————————————————————————————————————————————————————————————————————————————————————————
    trained_state_dict = torch.load('/home/zhonggai/python-work-space/DEFormer/DEFormer/scripts/work_dir_voc_wseg/test_best(不佳啊74.2)/checkpoints/default_model_iter_8000.pth', map_location="cpu")
...
# discrepancy loss ----------------------------------------------------------------
        ...
        # fmap_2_flat = b1_fmap.view(b1_fmap.shape[0], b1_fmap.shape[1], -1)

        # cos_simi = torch.nn.CosineSimilarity(dim=-1, eps=1e-6)
        # feature_sim_loss_1 = 1 + cos_simi(fmap_1_flat.detach(), fmap_2_flat).mean()
        # feature_sim_loss_2 = 1 + cos_simi(fmap_2_flat.detach(), fmap_1_flat).mean()

        # sim_loss = feature_sim_loss_1 + feature_sim_loss_2
...
#
        ...
        show_mask_cam(b1_mix_cams,cls_label,args.low_thre,args.high_thre, 'b1_mix_cam')
        show_mask_cam(b2_mix_cams,cls_label,args.low_thre,args.high_thre, 'b2_mix_cam')
