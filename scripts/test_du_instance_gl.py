...
from model.losses import get_masked_ptc_loss, get_seg_loss, ContrastLoss_prototype_instance, DenseEnergyLoss, get_energy_loss,CPCLoss,get_spacial_bce_loss,ContrastLoss_moco,ContrastLoss_prototype,get_bg_fg_contrastive_clamp_loss,get_bg_fg_contrastive_loss,get_masked_consistency_loss
from model.model_seg_neg import network
from model.double_seg_head import network_du_heads_independent_config_instance
...
from utils.camutils import single_instance_cropv3,single_bg_fg_crop_denoise_v3,cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads,select_local_patch
...
def train(args=None):
    ...
    model = network_du_heads_independent_config_instance(
...
#get_local_pic_to_encode------------------------------------------------------------------------------
        ...
        b1_local_crops, b1_flags, b1_box, b1_all_cls_image= single_instance_cropv3(images=inputs, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label)
        b2_local_crops, b2_flags, b2_box, b2_all_cls_image= single_instance_cropv3(images=inputs, cls_label = cls_label,roi_mask_28x=b2_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label)
        
        
        all_cls_crop = (b1_all_cls_image, b2_all_cls_image)
        local_pic = (b1_local_crops , b2_local_crops)
        # print(len(b1_local_crops))
# model forward-------------------------------------------------------------------------------------------------------------------------------

        cls, segs, fmap, cls_aux, cam_12th, cls_token, contrast_feature = model(inputs, all_cls_crop =all_cls_crop , n_iter=n_iter,select_k = 1,return_cls_token = True,local_pic = local_pic)    
        ...
        ((b1_single_cls_token,b1_all_cls_token),(b2_single_cls_token,b2_all_cls_token)) = contrast_feature
        # b1_contrast_feature = contrast_feature

#instance learning -----------------------------------------------------------------------

        if n_iter >= 3000:
            b1_prototyped_loss = b1_contrast_prototype(b1_cls_token,b1_single_cls_token,cls_label,b1_flags)
            b2_prototyped_loss = b2_contrast_prototype(b2_cls_token,b2_single_cls_token,cls_label,b2_flags)
            contrast_loss = (b1_prototyped_loss + b2_prototyped_loss)
            print("prototype",contrast_loss) 
            consistency_loss = get_masked_consistency_loss()
