...
from model.model_seg_neg_backup22 import network
...
from utils.camutils import cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg,cam_patch_contrast_loss,multi_scale_cam_test,crop_from_roi_posi,cam_to_label_resized  #,netural_cam_loss
...
parser.add_argument("--scales", default=(0.9, 1.1), help="random rescale in training")
...
parser.add_argument("--wt_decay", default=5e-3, type=float, help="weights decay")
...
def train(args=None):
    ...
    trained_state_dict = torch.load('/home/zhonggai/python-work-space/WSSS-work/Token-contrast/ToCo-main/scripts/work_dir_voc_wseg/without_ctc/checkpoints/default_model_iter_5000.pth', map_location="cpu")
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            ...
            train_loader_iter = iter(train_loader)
            img_name, inputs, cls_label, img_box, crops,image_origin = next(train_loader_iter)
            #img_name, image, cls_label, img_box, crops
            #crops怎么得到的？
            # if self.aug:
        ...
        #b h w
        local_crops, flags= crop_from_roi_neg(images=inputs, cls_label = cls_label,roi_mask=roi_mask, crop_num=ncrops-2, crop_size=args.local_crop_size)
        roi_crops = crops[:2] + local_crops #全局的两张图 + local的多张图
        # flag [b,num_crop]  roi_crops list(num_crop * [b,3,cs,cs])
        ...
        # cam_loss = netural_cam_loss(fmap=fmap,mask=roi_mask,cls_token = cls_token)
        ...
        aff_mask_aux = label_to_aff_mask(pseudo_label_aux)
        ptc_loss_1 = get_masked_ptc_loss(fmap_refined, aff_mask_aux)
        
        
        # resized_cams = F.interpolate(cams, size=fmap.shape[2:], mode="bilinear", align_corners=False)
        # _, pseudo_label_cam = cam_to_label_resized(resized_cams.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index,printornot=False,clip = True)
        # #伪标签是2 20 28 28，这个用于分出pair
        # aff_mask_cam = label_to_aff_mask(pseudo_label_cam)
        # ptc_loss_2 = get_masked_ptc_loss(fmap_refined, aff_mask_cam)
        ...
        if n_iter <= 2000:   
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc* ctc_loss + 0.1*cpc_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss +0.1*cpc_loss   # + 0.2*cam_contrast_loss
