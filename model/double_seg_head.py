...
class network_CL_gl(VisionTransformer):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=0.95, aux_layer=None):
        super().__init__()
        self.num_classes = num_classes
        self.init_momentum = init_momentum
        ...
        # #new add global-------------------------------------------------------------
        self.patch_proj_global = Fmap_proj_Head(in_dim=self.embed_dim, out_dim=1024)
        # # #----------------------------------------------------------------
    ...
    def _EMA_update_encoder_teacher(self, n_iter=None):
        ## no scheduler here
        momentum = self.init_momentum
        for param, param_t in zip(self.patch_proj.parameters(), self.patch_proj_global.parameters()):
            param_t.data = momentum * param_t.data + (1. - momentum) * param.data
    def get_param_groups(self):
        ...
        # new add------------------------------  
        for name, param in list(self.patch_proj_global.named_parameters()):
            param_groups[2].append(param)
        # ----------------------------------------------------------------
    ...
    def encode_local_pic(self, local_pic):
        ...
        token_proj_local = self.patch_proj(cls_token)
        return token_proj_local 
    ...
    def forward(self, x ,cam_mask=None ,cam_only=False, crops=None, n_iter=None,cam_crop = False,select_k = 1,return_cls_token = False, local_pic = None, local_flag=None):
        ...
        #only optimize backbone(classifier is only supervised by cls-loss)
        # with torch.no_grad():
        cam_12th = cam = F.conv2d(_x4, self.classifier.weight)
        ...
        cls_token = self.patch_proj_global(cls_token)
...
class network_instance_gl(VisionTransformer):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=0.95, aux_layer=None):
        super().__init__()
        self.num_classes = num_classes
        self.init_momentum = init_momentum
    ...
    def forward(self, x ,all_cls_crop=None ,cam_only=False, crops=None, n_iter=None,cam_crop = False,select_k = 1,return_cls_token = False, local_pic = None, local_flag=None):
        #x (b c h w) [2 3 448 448]
        cls_token ,_x, x_aux, embeds= self.forward_features(x)
        #cls-token final-patch  mid-patch
        #b 1 D     B N（784） D（768）         B N D
        ...
        if return_cls_token:
            if local_pic != None:
                single_cls_token = self.encode_local_pic(local_pic)
            if all_cls_crop !=None:
                all_cls_token = self.encode_local_pic(all_cls_crop)
            contrast_token_feature = (single_cls_token,all_cls_token)
        #only optimize backbone(classifier is only supervised by cls-loss)
        # with torch.no_grad():
        cam_12th = cam = F.conv2d(_x4, self.classifier.weight)
...
class network_du_heads_independent_config_cl_gl(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None, pretrained_path= None):
        super(network_du_heads_independent_config_cl_gl, self).__init__()

        self.branch1 = network_CL_gl(backbone, num_classes, pretrained, init_momentum, aux_layer)
        self.branch2 = network_CL_gl(backbone, num_classes, pretrained, init_momentum, aux_layer)
...
class network_du_heads_independent_config_instance(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None, pretrained_path= None):
        super(network_du_heads_independent_config_instance, self).__init__()

        self.branch1 = network_instance_gl(backbone, num_classes, pretrained, init_momentum, aux_layer)
        self.branch2 = network_instance_gl(backbone, num_classes, pretrained, init_momentum, aux_layer)
    ...
    def forward(self, x, all_cls_crop=None,cam_only=False, crops=None, n_iter=None,cam_crop = False,select_k = 1,return_cls_token = False, local_pic = None):
        if cam_only == True:
            b1_cam_aux, b1_cam  = self.branch1.forward(x, all_cls_crop, cam_only, crops, n_iter, cam_crop, select_k, return_cls_token,)
            b2_cam_aux, b2_cam  = self.branch2.forward(x, all_cls_crop, cam_only, crops, n_iter, cam_crop, select_k, return_cls_token,)
            return (b1_cam_aux, b2_cam_aux), (b1_cam, b2_cam)
        elif return_cls_token == False:
            b1_cls_x4, b1_seg, b1_x4, b1_cls_aux = self.branch1.forward(x, cam_only, crops, n_iter, cam_crop, select_k, )
            b2_cls_x4, b2_seg, b2_x4, b2_cls_aux = self.branch2.forward(x, cam_only, crops, n_iter, cam_crop, select_k, )            
            ...
            if return_cls_token == True:
                ...
                (b1_local_pic,b2_local_pic) = local_pic
                b1_cls_x4, b1_seg, b1_x4, b1_cls_aux, b1_cam12th, b1_cls_token, contrast_feature_b1 = self.branch1.forward(x,all_cls_crop[0], cam_only, '#', n_iter, cam_crop, select_k, return_cls_token,b1_local_pic)
                b2_cls_x4, b2_seg, b2_x4, b2_cls_aux, b2_cam12th, b2_cls_token, contrast_feature_b2 = self.branch2.forward(x,all_cls_crop[1], cam_only, '#', n_iter, cam_crop, select_k, return_cls_token,b2_local_pic)  
                return (b1_cls_x4,b2_cls_x4),(b1_seg,b2_seg), (b1_x4, b2_x4), (b1_cls_aux, b2_cls_aux), (b1_cam12th, b2_cam12th) , (b1_cls_token, b2_cls_token), (contrast_feature_b1,contrast_feature_b2)
            else:
                AssertionError("Error")
