        ...
        for i in range(b):
            ...
            for crop_idx in range(self.ncrops-2):
                ...
                #bg
                    ...
                    #uncertain
                        local_cls =local_output[i][crop_idx]
                        total_loss += F.multilabel_soft_margin_loss(local_cls,cls_input[i])
                        cnt+=1
...
class CPCLoss(nn.Module):
    ...
    def forward(self, fmap, cam,cls_label,hig_thre,low_thre,bg_thre):
        ...
        mask = torch.zeros(b,self.num_cls,h,w)
        ...
        for i in range(b):
            ...
            for cls,feature in feature_vector.items():
                ...
                # mask[i][cls] = torch.abs(torch.nn.functional.cosine_similarity(expanded_feature, fmap[i].detach(), dim=0))
            ...
            # mask_indices[mask_value<low_thre] = 0
            ...
            cos_smi = F.cosine_similarity(feature_single_map.unsqueeze(1),self.feature_contrast.unsqueeze(0),dim=-1)
            cos_smi = torch.abs(cos_smi)
            cos_smi = torch.clamp(cos_smi,min=1e-5,max=1-1e-5)
            cls_index = []
            for cls,feature in feature_vector.items():
                cls_index.append(cls)
            
            loss += F.binary_cross_entropy(cos_smi,indentity_matrix)
            self.feature_contrast[cls_index] = 0.95 * self.feature_contrast[cls_index] + 0.05 * feature_single_map[cls_index].detach()
            self.feature_contrast = F.normalize(self.feature_contrast,p=2,dim=-1)
            # print(cos_smi)
            print(torch.sum(abs(F.cosine_similarity(self.feature_contrast.unsqueeze(1),self.feature_contrast.unsqueeze(0),dim=-1)))-21)
