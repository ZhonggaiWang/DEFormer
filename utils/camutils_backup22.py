...
def cam_to_label(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None):
    ...
    if ignore_mid: #忽略中间部分high_thre是类别数 low_thre得是背景 中间得是忽略区域
    ...
    #torch.ones_like(_pseudo_label)创建了一个形状相同的张量，并将所有元素初始化为1
...
#             neighbor_counts = F.conv2d((roi_mask[i1]==class_mask_idx).float().unsqueeze(0).unsqueeze(0).float(), kernel.unsqueeze(0).unsqueeze(0), padding=0).squeeze(0).squeeze(0).long()
...
#             rand_index = torch.randperm(roi_index.shape[0])
#             #torch.randperm() 函数返回一个随机排列的整数序列，范围从 0 到 roi_index.shape[0] - 1。
#             crop_index = roi_index[rand_index[:crop_num], :] + torch.full((1, 2), margin).cuda()
...
#             #确定一个点
#             temp_crops[i1, i2, ...] = images[i1, :, (h0-margin):(h0+margin), (w0-margin):(w0+margin)]
#             #temp_crops 的形状将是 [b, n, 3, crop_size, crop_size]
#             #...用于省略连续的冒号
#             temp_mask = roi_mask[i1, (h0-margin):(h0+margin), (w0-margin):(w0+margin)].cpu()
...
#                         flags[i1, i2+2,element_idx+1] = 1
#             if torch.all(flags[i1,i2+2] == 0):
#                 for element, count in zip(sorted_unique_elements, sorted_counts):
#                     if element.item() >=0 and count.item()>100:
#                         element_idx = element.item()
#                         flags[i1,i2+2,element_idx+1] = 1
#     crop_fg_num = sum(crop_fg_num) / crop_num
#     select_k = int(crop_fg_num / (16*16))
...
#     #list [2 3 96 96]
#     return crops, flags.cuda(),select_k
...
#     # for i in range(20):
#     #     if torch.sum(valid_cam[0][i])!=0:
#     #         print(valid_cam[0][i])
#     #         plt.imshow(valid_cam[0][i].detach().cpu(), cmap='jet')
#     #         plt.colorbar()
#     #         plt.title(f'Activation Map - valid-cam {i}')
#     #         plt.savefig(f'activation_map_valid_cam{i}.png')
#     #         plt.close()
...
def multi_scale_cam_test(model, inputs, scales):
    ...
    with torch.no_grad():
        ...
        cam_list = [F.relu(_cam),F.relu(_cam_crop)]
        cam_aux_list = [F.relu(_cam_aux)]
        ...
        for s in scales:
            if s != 1.0:  #原图缩放
              ...
              #直接上采样  aug
                ...
                #b c h w
                cam_list.append(F.relu(_cam))
                cam_list.append(F.relu(_cam_crop))
                cam_aux_list.append(F.relu(_cam_aux))
        #torch.stack(cam_list, dim=0) 将 cam_list 中的 CAM 张量在新的维度 dim=0 上进行堆叠。
        # 这将创建一个形状为 (num_cams, b, c, h, w) 的新张量，
        # 其中 num_cams 是 CAM 的数量，b 是批量大小，c 是通道数，h 和 w 是 CAM 的高度和宽度。
...
def cam_patch_contrast_loss(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None,fmap=None):
    ...
    for i in range(b):
        ...
        similarity_matrix = torch.clamp(similarity_matrix, min=1e-5, max=1-1e-5)
        identity_matrix = torch.eye(similarity_matrix.size(0)).cuda()
        identity_matrix = torch.clamp(identity_matrix, min=1e-5, max=1-1e-5)
